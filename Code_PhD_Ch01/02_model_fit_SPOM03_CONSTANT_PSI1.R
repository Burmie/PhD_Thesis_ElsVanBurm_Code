...
metapop.model <- function(){
  ...
  for (i in 1:nsites){
    ...
    for(t in 2:nyears){
      ...
      logit(gamma[i, t]) <- alpha.gamma + beta.gamma2 * conn[t, i]
      # logistic model for the probability of extinction (epsilon) at each time step after the first
      logit(epsilon[i, t]) <- alpha.epsilon + beta.epsilon1 * effarea[i] + beta.epsilon2 * aqveg[i] + beta.epsilon3 * conn[t, i]
...
## fit model and save 
fit <- jags(data=y, inits=inits, parameters.to.save=c("alpha.psi","alpha.gamma","beta.gamma2","alpha.epsilon","beta.epsilon1","beta.epsilon2","beta.epsilon3"),
