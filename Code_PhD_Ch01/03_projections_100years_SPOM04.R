...
### Load distance matrix (previously calculated) ###
load("nbr.distweight")
diag(nbr.distweight) <- 0 
...
simulate_metapopulation_dynamics_100years <- function (run_name){ 
  ...
  ## Define files containing postburnin parameter values
	ag <- paste("../02_model_fit/SPOM04/",run_name,"_alpha.gamma.txt",sep="")
	ae <- paste("../02_model_fit/SPOM04/",run_name,"_alpha.epsilon.txt",sep="")
	bg2 <- paste("../02_model_fit/SPOM04/",run_name,"_beta.gamma2.txt",sep="")
	bg3 <- paste("../02_model_fit/SPOM04/",run_name,"_beta.gamma3.txt",sep="")
	be1 <- paste("../02_model_fit/SPOM04/",run_name,"_beta.epsilon1.txt",sep="")
	be2 <- paste("../02_model_fit/SPOM04/",run_name,"_beta.epsilon2.txt",sep="")
	be3 <- paste("../02_model_fit/SPOM04/",run_name,"_beta.epsilon3.txt",sep="")
	be4 <- paste("../02_model_fit/SPOM04/",run_name,"_beta.epsilon4.txt",sep="")
  
  ## Load parameter combinations
  alpha.gamma <- read.table(ag, header = T, sep=",")[,2]
  alpha.epsilon <- read.table(ae, header = T, sep=",")[,2]
  beta.gamma2 <- read.table(bg2, header = T, sep=",")[,2]
  ...
  beta.epsilon3 <- read.table(be3, header = T, sep=",")[,2]
  ...
  beta.gamma2 <- beta.gamma2[params_to_keep]
  ...
  cat("   running",ncombinations,"parameter combinations\n")
    ...
    metapopmodel <- function(alpha.gamma, alpha.epsilon, beta.gamma2, beta.gamma3, beta.epsilon1, beta.epsilon2, beta.epsilon3, beta.epsilon4){
      # Preallocate arrays
      ...
      # Dynamics
      for (t in 2:YEARS){
        # Calculate contribution of each neighbour to connectivity at each time-step and connectivity
        consim<- as.numeric(log(occ[t-1, ] %*% nbr.distweight +1))
        # Rainfall
        rainfall <- runif(1,200,600) 
        # Specify logistic model for the probability of colonisation
        logit.gam <- alpha.gamma + beta.gamma2*consim + beta.gamma3*rainfall
        ...
        logit.eps <- alpha.epsilon + beta.epsilon1*effareasim + beta.epsilon2*aqvegsim + beta.epsilon3*consim + beta.epsilon4*rainfall
    ...
    results <- sapply(1:ncombinations, function(x) {
      ...
      replicate(ITERS, metapopmodel(alpha.gamma[x], alpha.epsilon[x], beta.gamma2[x], 
                                    beta.gamma3[x], beta.epsilon1[x], beta.epsilon2[x], 
                                    beta.epsilon3[x], beta.epsilon4[x]))
    })
